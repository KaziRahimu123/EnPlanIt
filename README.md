...
EnPlanIt is a unified aerospace mission intelligence and scenario simulation platform built for mission architects, systems engineers, flight operations teams, mission risk and safety analysts, and space exploration planners. When a mission designer uploads a spaceflight proposal, flight specification, telemetry brief, or concept dossier in PDF, DOCX, TXT, or Markdown format (up to 3 files / 20 MB simultaneously), EnPlanIt processes the material using a hybrid AI pipeline of IBM Granite 20B (ibm/granite-20b-multilingual via watsonx.ai) and GPT-5.6 Luna, dynamically extracting verified telemetry channels, mapping cross-subsystem dependencies, and generating structured mission action plans (Objectives, Major Constraints, Resource Demands, and Operational Directives) in seconds, complete with an Interactive Orbital Topological Map and a Deterministic Scenario Lab to stress-test cascading flight risks—from battery discharge kinetics to life-support consumable depletion—grounded in real aerospace physics.

> **Built for the IBM AI Builders Challenge (August 2026, Advance Space Exploration with AI).**

[Try the live demo](https://enplanit-web.vercel.app/) · [Judge's Quick Guide](./JUDGE.md) · [Watch the 3-minute demo](https://youtu.be/-xclATddQx0?si=UXPeeEQ0FYvRPQoN)
...
| **Verification pipeline** | **Hybrid Dual-Pass AI Pipeline**: Uses **IBM Granite 20B (`ibm/granite-20b-multilingual`) on watsonx.ai** for primary telemetry extraction and synthesis, coupled with **GPT-5.6 Luna** for secondary cross-verification against aerospace flight standards. |
| **Scenario simulation** | Evaluates changes involving power availability, battery reserves, mission duration, communication delay, crew resources, and environmental constraints using deterministic physics. |
| **Built with** | **Hybrid AI Pipeline (IBM Granite 20B on watsonx.ai + GPT-5.6 Luna)** · **IBM Bob** · **Next.js 16** · **React 19** · **Tailwind CSS v4** · **FastAPI** · **Supabase** · **Auth0** · **TypeScript** · **Python** |
...
flowchart TD
    ...
    subgraph Intelligence["Hybrid Intelligence Layer"]
        Granite["Primary: IBM Granite 20B on watsonx.ai"]
        Verifier["Secondary Verifier: GPT-5.6 Luna"]
        Physics["Deterministic Physics & Aerospace Logic"]
    end
...
| **AI pipeline** | **Hybrid AI Pipeline**: **IBM Granite 20B (`ibm/granite-20b-multilingual`) on watsonx.ai** performs primary parameter extraction and flight synthesis. **GPT-5.6 Luna** acts as a cross-verification engine to ensure constraint accuracy and reduce hallucinations. |
...
- **AI Pipeline Architecture:** Helped design the hybrid dual-pass extraction flow using IBM Granite 20B on watsonx.ai paired with GPT-5.6 Luna for secondary verification, structured JSON outputs, and deterministic aerospace fallback.
